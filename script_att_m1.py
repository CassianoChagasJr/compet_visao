...
fps_criacao = 20 #FPS
largura_criacao, altura_criacao = 320, 240 #Dimensões do video
nome_arquivo_video_criado = "video_atividade_m1.mp4" #Nome do arquivo
...
for i in range(num_quadros_criacao):
    quadro = np.full((altura_criacao, largura_criacao, 3), 255, dtype=np.uint8) # Cria um quadro branco
    x_pos = int(30 + (largura_criacao - 60) * i / num_quadros_criacao)  # Calcula a posição X do círculo para criar um movimento
...
if not captura.isOpened():
 ...
 # Calcula o índice do quadro do meio
    ...
    cv2.imwrite(nome_arquivo_quadro_meio, quadros_lidos[indice_quadro_meio]) # Salva o quadro do meio como imagem

    # Carrega e exibe o quadro do meio
    quadro_meio_para_exibir = cv2.imread(nome_arquivo_quadro_meio)
    print("\nQuadro do meio:")

    # Código para exibição
    cv2.imshow('Quadro do Meio', quadro_meio_para_exibir)
